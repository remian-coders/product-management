...
export const setToggleScheduler = async function (token, data) {
  try {
    let response = await api.patch(`/admin/toggle-scheduler`, data, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response;
  } catch (error) {
...
export const getToggleScheduler = async function (token) {
  try {
    let response = await api.get(`/admin/toggle-scheduler`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response;
  } catch (error) {
